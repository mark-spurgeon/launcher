...
class Container(QWidget):
	...
	def paintEvent(self, event):
		...
		qp.setPen(QPen(QColor(70,70,70),2))
		qp.drawRoundedRect(QRect(0,0,self.width(),self.height()), 6,6)
		#title
		qp.setPen(QPen(QColor(250,250,250),2))
		qp.setFont(QFont("Droid Sans",11, QFont.Normal))
		t_rect=QRect(10,0,160,35)
		qp.drawText(t_rect, Qt.AlignCenter, "Duck Launcher Settings")
		#
		if self.closed_pressed==True:
		
			i = QIcon("/usr/share/duck-launcher/default-theme/settings-close-pressed.svg")
			i.paint(qp, self.width()-40,0,40,40)
		else:
			i = QIcon("/usr/share/duck-launcher/default-theme/settings-close.svg")
			i.paint(qp, self.width()-40,0,40,40)			
		#
	...
	def mousePressEvent(self, e):
		self.offset =e.pos()
		if e.x()>self.width()-40 and e.y()<40:
	...
	def mouseMoveEvent(self, e):
		...
		if self.width()<200:
			...
			self.view.resize(200,self.height()-60)
			self.resizing=False
		if self.height()<200:
			self.resize(self.width(),202)
			self.view.resize(self.width()-2,140)
			self.resizing=False
...
if __name__=="__main__":
	...
	t=Template(f.read())
	f.close()
	w.setHtml(t.render(cfg=cfg,fonts=fonts,apps=apps,static_url="file:///usr/share/duck-launcher/default-theme/"))#file:///usr/share/duck-launcher/default-theme/
	w.show()
	sys.exit(app.exec_())
